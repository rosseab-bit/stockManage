...
class dbSqlite:
    ...
    def updateStock(self, producto):
        ...
        print(producto)
        cur=self.sqliteConnection.cursor()
        updateQuery='update Stock set Stock=%s, Precio=%s, Descripcion=\'%s\', FeachaVencimiento=\'%s\', FeachaDeCompra=\'%s\' where Codigo=%s;'%(producto['stock'], producto['precio'], producto['descripcion'], producto['codigo'], producto['fecha_vencimiento'], producto['fecha_compra'])
        print(updateQuery)
    ...
    def addItemStock(self, item):
        ...
        print(item)
        cur=self.sqliteConnection.cursor()
        addItem="insert into Stock (Codigo, Stock, Precio, Descripcion, FeachaVencimiento, FeachaDeCompra) values (%s, %s, %s, '%s')"%(item['codigo'], item['stock'], item['precio'], item['descripcion'], item['fecha_vencimiento'], item['fecha_compra'])
        print(addItem)
        cur.execute(addItem)
        self.sqliteConnection.commit()
